...
public class PlayerManagementItem extends MxItem {
        ...
        game.getColors().forEach(((gamePlayer) -> {
            ...
            list.add(new Pair<>(
                    MxSkullItemStackBuilder.create(1)
                            ...
                            .setName(gamePlayer.getPlayer().isPresent() && Bukkit.getPlayer(gamePlayer.getPlayer().get()) != null ? "<gray>" + Bukkit.getPlayer(gamePlayer.getPlayer().get()).getName() : "<gray>Geen speler toegewezen")
                            .addBlankLore()
                            .addLore("<gray>Kleur: " + gamePlayer.getMapPlayer().getColor().getDisplayName())
                            .addLore("<gray>Rol: " + gamePlayer.getMapPlayer().getRoleDisplayString())
                            .addBlankLore()
                            .addLore("<yellow>Klik om deze kleur aan te passen.")
                            .build(),
                    (mxInv, e1) -> {
                        MxInventoryManager.getInstance().addAndOpenInventory(p, MxDefaultMenuBuilder.create("<gray>Beheer kleur: " + gamePlayer.getMapPlayer().getColor().getDisplayName(), MxInventorySlots.THREE_ROWS)
                                .setItem(MxDefaultItemStackBuilder.create(Material.BOOK)
                                                .setName("<gray>Wijzig rol")
                                                .addBlankLore()
                                                .addLore("<gray>Huidig: " + gamePlayer.getMapPlayer().getRoleDisplayString())
                                                .addBlankLore()
                                ...
                                .setItem(MxDefaultItemStackBuilder.create(Material.SKELETON_SKULL)
                                                ...
                                                .addBlankLore()
                                                .addLore("<gray>Huidig: " + (gamePlayer.getPlayer().isPresent() ? Bukkit.getPlayer(gamePlayer.getPlayer().get()).getName() : "Geen speler"))
                                                .addBlankLore()
                                        ...
                                        (mxInv1, e2) -> {
                                            ...
                                            MxInventoryManager.getInstance().addAndOpenInventory(p, new MxListInventoryBuilder("<gray>Kies speler (" + gamePlayer.getMapPlayer().getColor().getDisplayName() + "<gray>)", MxInventorySlots.FOUR_ROWS)
                                                    ...
                                                    .setItem(MxDefaultItemStackBuilder.create(Material.NAME_TAG)
                                                            ...
                                                            (mxInv2, e3) -> {
                                                                p.closeInventory();
                                                                MSG.msg(p, LanguageManager.getInstance().getLanguageString(LanguageText.GAME_ENTER_NAME, ChatPrefix.WIDM));
                                                                MxChatInputManager.getInstance().addChatInputCallback(p.getUniqueId(), message -> {
                                                                    Bukkit.getScheduler().runTaskLater(plugin, () -> {
                                                                        ...
                                                                        if (enteredPlayer == null) {
                                                                            MSG.msg(p, LanguageManager.getInstance().getLanguageString(LanguageText.GAME_ENTER_NAME_NOT_FOUND, ChatPrefix.WIDM));
                                                                            return;
                                                                        }
                                                                        if (!game.getGameInfo().getQueue().contains(enteredPlayer.getUniqueId())) {
                                                                            MSG.msg(p, LanguageManager.getInstance().getLanguageString(LanguageText.GAME_ENTER_NAME_NOT_IN_QUEUE, ChatPrefix.WIDM));
                                                                            return;
                                                                        }
                                                    ...
                                                    .setItem(MxDefaultItemStackBuilder.create(Material.SKELETON_SKULL)
                                                        ...
                                                        p.closeInventory();
                                                        MSG.msg(p, LanguageManager.getInstance().getLanguageString(LanguageText.GAME_COLOR_CLEARED, Collections.singletonList(gamePlayer.getMapPlayer().getColor().getDisplayName())));
                                                    })
                                                    .build());
                                        })
                                .setItem(MxDefaultItemStackBuilder.create(Material.GOLDEN_SWORD)
                                        ...
                                        (mxInv1, e2) -> {
                                            ...
                                            p.closeInventory();
                                            MSG.msg(p, LanguageManager.getInstance().getLanguageString(LanguageText.GAME_PLAYERSTATE_CHANGED, Arrays.asList(gamePlayer.getMapPlayer().getColor().getDisplayName(), gamePlayer.isAlive() ? "<green>Levend" : "<red>Dood")));
                                            if (gamePlayer.getPlayer().isEmpty())
                                                return;
    ...
    private ItemStack getRolTypeItemStack(MapPlayer mapPlayer) {
        return MxDefaultItemStackBuilder.create(Material.BOOK)
                .setName("<gray>Wijzig rol")
                .addBlankLore()
                .addLore("<gray>Huidig: " + mapPlayer.getRoleDisplayString())
                .addBlankLore()
                .addLore("<yellow>Klik hier om de rol aan te passen.")
                .build();
    }
