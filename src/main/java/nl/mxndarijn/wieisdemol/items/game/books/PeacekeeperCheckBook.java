            ...
            game.getColors().forEach(gamePlayer -> {
                ...
                list.add(new Pair<>(
                        MxSkullItemStackBuilder.create(1)
                                ...
                                .addBlankLore()
                                .addLore("<yellow>Klik hier om te kijken of")
                                .addLore("<yellow>" + player.getName() + "de peacekeeper is.")
                                .build(),
                        (mxInv, e1) -> {
                            p.closeInventory();
                            for (Map.Entry<Integer, ? extends ItemStack> entry : p.getInventory().all(is.getType()).entrySet()) {
                                ...
                                if (isItemTheSame(value)) {
                                    if(!canItemExecute(p, key, value, BookFailurePlayersHolder.create().setData(AvailablePerson.EXECUTOR, p)))
                                        return;
                                    String peacekeeper = gamePlayer.getMapPlayer().isPeacekeeper() ? "<green>is" : "<red>is niet";

                                    // Check if book is silenced
                                    if (isSilenced(value)) {
                                        game.sendMessageToHosts(String.format("<gray>[SILENT] <white>", LanguageManager.getInstance().getLanguageString(LanguageText.GAME_PEACEKEEPER_CHECK_MESSAGE, Arrays.asList(gp.getMapPlayer().getColor().getColor() + p.getName(), gamePlayer.getMapPlayer().getColor().getColor() + player.getName(), peacekeeper))));
                                        MSG.msg(p, String.format("<gray>[SILENT] <white>", LanguageManager.getInstance().getLanguageString(LanguageText.GAME_PEACEKEEPER_CHECK_MESSAGE, Arrays.asList(gp.getMapPlayer().getColor().getColor() + p.getName(), gamePlayer.getMapPlayer().getColor().getColor() + player.getName(), peacekeeper))));
                                    } else {
