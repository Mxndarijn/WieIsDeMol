...
public class PresetsCommand extends MxCommand {
    ...
    public void execute(CommandSender sender, Command command, String label, String[] args) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(JavaPlugin.getPlugin(WieIsDeMol.class), () -> {
                if (e1.getCurrentItem() != null) {
                    ...
                    if (optionalPreset.isPresent()) {
                        ...
                        p.closeInventory();
                        MSG.msg(p, ChatPrefix.WIDM + LanguageManager.getInstance().getLanguageString(LanguageText.COMMAND_PRESETS_LOADING_WORLD, Collections.emptyList()));
                        preset.loadWorld().thenAccept(loaded -> {
                            if (loaded) {
                                ...
                                if (w != null) {
                                    ...
                                    MSG.msg(p, ChatPrefix.WIDM + LanguageManager.getInstance().getLanguageString(LanguageText.COMMAND_PRESETS_NOW_IN_PRESET, Collections.emptyList()));
                                } else {
                                    MSG.msg(p, ChatPrefix.WIDM + LanguageManager.getInstance().getLanguageString(LanguageText.COMMAND_PRESETS_WORLD_NOT_FOUND_BUT_LOADED, Collections.emptyList()));
                                }
                            }
                        });
                    } else {
                        MSG.msg(p, ChatPrefix.WIDM + LanguageManager.getInstance().getLanguageString(LanguageText.COMMAND_PRESETS_WORLD_COULD_NOT_BE_LOADED, Collections.emptyList()));
                    }
                }
            });
        };
        ...
        MxInventoryManager.getInstance().addAndOpenInventory(p, MxListInventoryBuilder.create("<gray>Configureer presets", MxInventorySlots.SIX_ROWS)
